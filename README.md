...
Obsidian 插件：一键把当前活动 Markdown 文件导出为 PDF。
...
2. 启用 `PDF Ready Export`。
3. 打开要导出的 Markdown 文件。
4. 点击左侧 ribbon 的导出按钮，或打开命令面板执行 `PDF Ready Export: Export active Markdown to PDF`。
...
ln -s /path/to/pdf-ready-export /path/to/vault/.obsidian/plugins/pdf-ready-export
```

或者直接复制：

```bash
cp -R /path/to/pdf-ready-export /path/to/vault/.obsidian/plugins/pdf-ready-export
```
