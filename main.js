...
module.exports = class PdfReadyExportPlugin extends Plugin {
  async onload() {
  ...
  async exportFile(file, overrides = {}) {
    try {
      ...
      console.error("PDF Ready Export failed", error);
  ...
  async renderMarkdown(file, markdown) {
    ...
    root.className = "markdown-reading-view pdf-ready-export-render-root";
